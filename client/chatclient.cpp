...
void ChatClient::onConnected() {
    if (!m_clientSocket->waitForEncrypted(5000)) {  // Wait up to 5 seconds for the connection
        qWarning() << "SSL handshake failed:" << m_clientSocket->errorString();
        return;
    }
...
void ChatClient::sendMessage(const QString &text, const QString &command)
{
    if(!m_clientSocket->isEncrypted()) {
        m_clientSocket->waitForEncrypted(2000);
    }
    qDebug() << "send message";
    if (text.isEmpty())
...
void ChatClient::connectToServer(const QString &address, quint16 port)
{
    ...
    if (!certFile.open(QIODevice::ReadOnly)) {
        qWarning() << "Error loading certificate file:" << certFile.errorString();
        return;
    }
    ...
    m_clientSocket->setPeerVerifyMode(QSslSocket::VerifyNone); // For testing purposes
    m_clientSocket->setProtocol(QSsl::TlsV1_3);
    sslConfig.addCaCertificate(cert);
    m_clientSocket->setSslConfiguration(sslConfig);

    qDebug() << "connecting ...";
    m_clientSocket->connectToHostEncrypted(address, port);
}
